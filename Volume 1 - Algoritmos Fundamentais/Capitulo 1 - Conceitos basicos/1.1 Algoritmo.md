...
No Algoritmo E, a definitude do passo E1 exige que o leitor compreenda exatamente o significado da divisão de $$ m $$ por $$ n $$ e do cálculo do resto. No entanto, não há um consenso universal sobre esses conceitos para valores que não sejam inteiros positivos. Por exemplo, qual seria o resto de $$-8$$ dividido por $$-\pi$$? Ou de $$ 59/13 $$ dividido por zero? Portanto, o critério de definitude requer que $$ m $$ e $$ n $$ sejam sempre inteiros positivos ao executar o passo E1. Essa condição é garantida inicialmente, e, após o passo E1, $$ r $$ é um número inteiro não negativo que só pode ser zero no passo E3. Assim, $$ m $$ e $$ n $$ permanecem inteiros positivos, como necessário.  
...
Definimos formalmente um método computacional como sendo um quádruplo (Q, I, Ω, f), no qual Q é um conjunto contendo os subconjuntos I e Ω, e f é uma função de Q em si mesmo. Além disso, f deve deixar Ω fixo ponto a ponto; ou seja, f(q) deve ser igual a q para todos os elementos q de Ω. As quatro quantidades Q, I, Ω, f são destinadas a representar, respectivamente, os estados da computação, a entrada, a saída e a regra computacional. Cada entrada x no conjunto I define uma sequência computacional, x0, x1, x2, ..., conforme segue:
...
$$f(m, n, r, 1) = (m, n,$$ resto de $$m$$ dividido por $$n, 2);$$
$$f(m, n, r, 2) = (n) $$\quad se$$ r = 0,\quad (m, n, r, 3)\quad $$ caso contrário;
$$f(m, n, p, 3) = (n, p, p, 1).$$
